...
function App() {
  ...
  const players = [...new Set(MATCHES.flatMap((match) => [match.player1, match.player2]))].sort()
  ...
  return (
    <main className="app-shell">
      ...
      {isFilterOpen && <div className="filter-panel" id="match-filters">
        <label>Player<select value={playerFilter} onChange={(event) => setPlayerFilter(event.target.value)}><option value="">All players</option>{players.map((player) => <option key={player} value={player}>{player}</option>)}</select></label>
