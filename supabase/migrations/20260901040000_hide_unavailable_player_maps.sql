-- Keep unavailable responses so maps owned only by unavailable players can be excluded.
...
as $$
  ...
  group by maps.id, maps.name, attendee_names.attending, attendee_names.unavailable, attendee_names.unavailable_ids, limits.count, limits.vote_limit, limits.map_count
  order by maps.name;
$$;
